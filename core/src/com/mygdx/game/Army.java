...
public class Army extends Troop {
    ...
    public Army(int force, int health) {
        this.force = force;
        this.health = health;
    ...
    public int getForce() {
        ...
        for (String division: divisions.keySet()) {
            switch (division) {
                case "artillery":
                    force += divisions.get(division)* Values.ARTILLERY_FORCE;
                    break;
                case "tank":
                    force += divisions.get(division)* Values.TANK_FORCE;
                    break;
                case "infantry":
                    force += divisions.get(division)* Values.INFANTRY_FORCE;
                    break;
            }
        }
        return force;
        ...
        if (damage >= health) {
            ...
            while (damage > 0) {
                if (divisions.size() > 0) {
                    ...
                    switch (division) {
                        case "artillery":
                            hp = total * Values.ARTILLERY_HEALTH;
                            break;
                        case "tank":
                            hp = total * Values.TANK_HEALTH;
                            break;
                        case "infantry":
                            hp = total * Values.INFANTRY_HEALTH;
                            break;
                    }
                    if (damage > hp)
    ...
    public boolean canPlace(Region region) {
        return region.isLandAccess() && region.getCountryDivisions(country.getName()) < Values.MAX_DIVISIONS;
    }
